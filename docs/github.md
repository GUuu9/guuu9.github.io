...
## 7. Git 태그(Tag) 관리 및 GitHub Release

소프트웨어 배포(릴리즈) 시점에 코드 상태를 고정해두고 싶을 때 Git 태그를 사용하며, 이를 GitHub의 Release 기능과 연동하여 다운로드 패키지를 관리할 수 있습니다.

### 7.1 로컬에서 태그 관리
* **태그 생성**: 현재 커밋 시점에 태그를 생성합니다.
  ```bash
  git tag v0.0.0
  ```
* **태그 목록 조회**:
